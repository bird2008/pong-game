...
function isOnTheTopEdgePaddle() {
  return (
    ballX === (ballX - BALL_R < 40 || ballX + BALL_R < CANVAS_WIDTH - 40) &&
    (ballY - BALL_R === p1PaddleY || ballY - BALL_R === p2PaddleY)
  );
}
...
function isOnTheBottomEdgePaddle() {
  return (
    ballX === (ballX - BALL_R < 40 || ballX + BALL_R < CANVAS_WIDTH - 40) &&
    (ballY + BALL_R === p1PaddleY - PADDLE_HEIGHT ||
      ballY + BALL_R === p2PaddleY - PADDLE_HEIGHT)
  );
}
...
function shouldBounceFromLeftPaddle() {
  return (
    ...
    (isBallOnTheSameHeightAsPaddle(p1PaddleY) ||
      isOnTheTopEdgePaddle() ||
      isOnTheBottomEdgePaddle())
  );
}
